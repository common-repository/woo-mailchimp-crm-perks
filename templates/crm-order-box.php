...
ul.vx_notes li.customer-note .note_content:after {
  ...
  ?>
  <input type="hidden" name="vx_debug" value="<?php echo esc_html($_GET['vx_debug']) ?>">
  <?php
  }
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo esc_attr($this->id) ?>_send" title="<?php esc_html_e("Send to MailChimp",'woo-mailchimp-crm-perks')?>" value="yes"><?php esc_html_e("Send to MailChimp",'woo-mailchimp-crm-perks')?></button>
  <?php
      if($comments ){
  $log_url=admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$post_id); 
  ?>
  <a href="<?php echo esc_url($log_url) ?>" class="button" title="<?php esc_html_e("Go to Logs",'woo-mailchimp-crm-perks')?>"><?php esc_html_e("Go to Logs",'woo-mailchimp-crm-perks')?></a>
  <?php
      }
  ?>
  </p>