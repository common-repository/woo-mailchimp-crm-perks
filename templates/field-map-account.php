...
.crm_panel .vx_error{
  ...
  <div id="crm_fields_map" class="vx_contents">
    <div class="vx_div ">
  <div class="vx_head ">
<div class="crm_head_div"> <?php esc_html_e('1. Select MailChimp Account',  'woo-mailchimp-crm-perks' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vxc_account"><?php esc_html_e('Select MailChimp Account.', 'woo-mailchimp-crm-perks');  $this->tooltip($tooltips['sel_object']); ?></label>
  </div>
  <div class="vx_col2">
  <div>
  ...
  <div id="crm_ajax_account" style="display:none; text-align: center; line-height: 100px;"><i><?php esc_html_e('Loading, Please Wait...', 'woo-mailchimp-crm-perks'); ?></i></div>
    <?php 
    $valid=true;
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php esc_html_e('This is a required field','woo-mailchimp-crm-perks') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  $.post(ajaxurl,{action:'refresh_data_<?php echo esc_attr($this->id) ?>',vx_crm_ajax:vx_crm_ajax,vx_action:action,account:account,object:object,post_id:post_id},function(res){
 ...
 if(re.status == "ok"){
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php esc_html_e('Please Select Field Name','woo-mailchimp-crm-perks') ?>');
     return;   
    }  
...
if(field.label){
 temp.find('.crm_text_label').text(field.label);
  ...
  e.preventDefault(); 
  if(!confirm('<?php esc_html_e('Are you sure to remove ?','woo-mailchimp-crm-perks') ?>')){
      return;
  }
...
start_tooltip();
  ...
  if(object == ""){
  alert("<?php esc_html_e('Please Select Object','woo-mailchimp-crm-perks') ?>");
  return;
  }
  if(vx_ajax && vx_ajax.abort){
    vx_ajax.abort();  
...
$('#vx_add_fields_select').select2({ placeholder: '<?php esc_html_e('Select Field','woo-mailchimp-crm-perks') ?>'});
add_note_sel();
} 
  });
function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php esc_html_e('Select Field','woo-mailchimp-crm-perks') ?>'});
}  
