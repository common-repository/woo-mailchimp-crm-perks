...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <h2 style="padding-bottom: 12px; line-height: 36px">  <img alt="<?php esc_html_e("MailChimp Feeds", 'woo-mailchimp-crm-perks') ?>" title="<?php esc_html_e("MailChimp Log", 'woo-mailchimp-crm-perks') ?>" src="<?php echo esc_attr(self::$base_url)?>images/<?php echo esc_attr($this->crm_name) ?>-crm-logo.png" style="float:left; margin:0 7px 0px 0;" width="60" /> <?php esc_html_e("MailChimp Log", 'woo-mailchimp-crm-perks'); ?></h2>
...
<form id="vx_form" class="crm_form" method="get"><div>
  ...
  <input type="text" placeholder="<?php esc_html_e('Search','woo-mailchimp-crm-perks') ?>" value="<?php echo esc_attr($this->post('search')) ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php esc_html_e('All Objects','woo-mailchimp-crm-perks') ?></option>
  <?php
  ...
  <option value=""><?php esc_html_e('All Status','woo-mailchimp-crm-perks') ?></option>
  <?php    
  ...
  <option value=""><?php esc_html_e('All Times','woo-mailchimp-crm-perks') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php esc_html_e('From Date','woo-mailchimp-crm-perks') ?>" value="<?php echo esc_attr($this->post('start_date'));?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php echo esc_attr($this->post('end_date'));?>" placeholder="<?php esc_html_e('To Date','woo-mailchimp-crm-perks') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" class="button-secondary button crm_input_inline" name="search" title="<?php esc_html_e('Search','woo-mailchimp-crm-perks') ?>"><i class="fa fa-search"></i> <?php esc_html_e('Search','woo-mailchimp-crm-perks') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php esc_html_e('Apply','woo-mailchimp-crm-perks') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php esc_html_e('Apply','woo-mailchimp-crm-perks') ?></button>

  <?php          if(count($results)>0){
  ?>
  <button type="button" name="tab_action" id="vx_export" title="<?php esc_html_e('Export as CSV','woo-mailchimp-crm-perks') ?>" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php esc_html_e('Export as CSV','woo-mailchimp-crm-perks') ?></button> 
  <?php
  ...
  if(isset($results[0]['order_id']) && $results[0]['order_id']!=""){
     ...
     $parent_id=(int)$results[0]['parent_id'];
      ?>
  <a href="<?php echo esc_url(admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$order_id));?>" title="<?php echo sprintf(__('View Order# %s Logs','woo-mailchimp-crm-perks'),esc_attr($order_id)); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php 
  if($parent_id<0){
  echo sprintf(__('View Visitor# %s Logs','woo-mailchimp-crm-perks'),esc_attr($order_id)); 
  }else{
  echo sprintf(__('View Order# %s Logs','woo-mailchimp-crm-perks'),esc_attr($order_id)); 
  }
   ?></a><?php
  }}
 if($this->post('order_id') !="" || $this->post('id') !=""){
          ?><a href="<?php echo esc_url(admin_url( 'admin.php?page='.$this->id.'_log'));?>" title="<?php esc_html_e('View All Logs','woo-mailchimp-crm-perks') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php esc_html_e('View All Logs','woo-mailchimp-crm-perks') ?></a>        
          <?php
      }
  ?>
  <div class="tablenav-pages">
  <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','woo-mailchimp-crm-perks') ?> <span id="paging_range_min_header"><?php echo esc_attr($range_min)?></span> - <span id="paging_range_max_header"><?php echo esc_attr($range_max); ?></span> of <span id="paging_total_header"><?php echo esc_attr($items);?></span></span>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php esc_html_e('MailChimp ID', 'woo-mailchimp-crm-perks') ?>
  <i class="fa fa-caret-<?php echo esc_attr($crm_order) ?> vx_sort_icon <?php echo esc_attr($crm_class) ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="order_id"><?php esc_html_e('Order ID', 'woo-mailchimp-crm-perks') ?>
  <i class="fa fa-caret-<?php echo esc_attr($entry_order) ?> vx_sort_icon <?php echo esc_attr($entry_class) ?>"></i>                      
  </th>
  <th scope="col" class="manage-column"  data-name="object"><?php esc_html_e('Description', 'woo-mailchimp-crm-perks') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php esc_html_e('Time', 'woo-mailchimp-crm-perks') ?>
  <i class="fa fa-caret-<?php echo esc_attr($time_order) ?> vx_sort_icon <?php echo esc_attr($time_class) ?>"></i>
  </th>
  <th style="width: 40px"><?php esc_html_e('Detail','woo-mailchimp-crm-perks') ?></th>
  </tr>
  </thead>
  ...
  <th><?php esc_html_e('Detail','woo-mailchimp-crm-perks') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo esc_attr($row['id']) ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo esc_attr(self::$base_url) ?>images/<?php echo esc_attr($row["status_img"]) ?>.png" alt="<?php echo $row['status'] ? __('Active', 'woo-mailchimp-crm-perks') : __('Inactive', 'woo-mailchimp-crm-perks');?>" title="<?php echo esc_html($row['title']);?>" class="crm_status_img" /></td>
  <td class="column-name" style="width:40%"><?php echo wp_kses_post($row['a_link']) ?></td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php esc_html_e('Expand Detail','woo-mailchimp-crm-perks'); ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php esc_html_e('No Record(s) Found', 'woo-mailchimp-crm-perks'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php esc_html_e('Clear MailChimp Log','woo-mailchimp-crm-perks') ?>" href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce')); ?>"><?php esc_html_e('Clear MailChimp Log','woo-mailchimp-crm-perks') ?></a>
  <div class="tablenav-pages">   <span id="paging_header" class="displaying-num"><?php esc_html_e('Displaying','woo-mailchimp-crm-perks') ?> <span id="paging_range_min_header"><?php echo esc_attr($range_min)?></span> - <span id="paging_range_max_header"><?php echo esc_attr($range_max); ?></span> of <span id="paging_total_header"><?php echo esc_attr($items);?></span></span>
  <?php  echo wp_kses_post($page_links);?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php esc_html_e('Please Select Action','woo-mailchimp-crm-perks') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php esc_html_e('Please select at least one entry','woo-mailchimp-crm-perks') ?>');
    return false;
}
var action=sel.val();
if( $.inArray(action,["send_to_crm_bulk_force","send_to_crm_bulk"]) !=-1 && $(".crm_input_check:checked").length>4){
 if(!confirm('<?php esc_html_e('Exporting more than 4 entries may take too long. Are you sure you want to continue?','woo-mailchimp-crm-perks') ?>')){
  e.preventDefault();    
 }   
}
  })
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php esc_html_e('MailChimp Logs will be deleted permanently. Do you want to continue?','woo-mailchimp-crm-perks') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(is_main){
    icon.attr('title','<?php esc_html_e('Expand Detail','woo-mailchimp-crm-perks'); ?>');    
    }        
     return;   
    }else{
        next_tr.show('fast'); 
    next_tr.find('.vxa_entry').slideDown('fast');
       if(is_main){
    icon.attr('title','<?php esc_html_e('Collapse Detail','woo-mailchimp-crm-perks'); ?>');    
    }    
    }
